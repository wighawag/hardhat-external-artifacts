...
export default async (): Promise<Partial<ConfigHooks>> => {
  const handlers: Partial<ConfigHooks> = {
    resolveUserConfig: async (userConfig, resolveConfigurationVariable, next) => {
      ...
      const externalArtifactsUserConfig = userConfig.externalArtifacts as ExternalArtifactsConfig | undefined;

      // Apply defaults
      const externalArtifacts: Required<Omit<ExternalArtifactsConfig, "resolver">> & {
