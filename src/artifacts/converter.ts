 ...
 */
function richArtifactToCompilation(artifact: RichArtifact): SyntheticCompilation | null {
  ...
  compilerOutput.contracts[sourceName][artifact.contractName] = {
    ...
    evm: {
      ...
      methodIdentifiers: artifact.evm?.methodIdentifiers ?? computeMethodIdentifiers(artifact.abi),
    },
 ...
 */
function synthesizeCompilation(artifacts: ExternalArtifact[], solcVersion: string): SyntheticCompilation {
...
function getCanonicalType(param: any): string {
  if (param.type === "tuple" || param.type === "tuple[]") {
    ...
    const tupleTypes = components.map((c: any) => getCanonicalType(c)).join(",");
