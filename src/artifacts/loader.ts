import type { ExternalArtifact, RichArtifact, ExternalArtifactsConfig } from "./types.js";
...
export class ArtifactLoader {
  ...
  #normalizeArtifact(raw: any, source: string): ExternalArtifact {
    ...
    if (!raw.contractName || typeof raw.contractName !== "string") {
      throw new Error(`Artifact from ${source} is missing required field 'contractName'`);
    }
    if (!raw.sourceName || typeof raw.sourceName !== "string") {
      throw new Error(`Artifact from ${source} is missing required field 'sourceName'`);
    }
